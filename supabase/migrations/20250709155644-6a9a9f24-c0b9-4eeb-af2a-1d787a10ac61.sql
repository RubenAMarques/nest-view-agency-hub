...
-- Create imports table for tracking XML uploads (must be before listings due to foreign key)
...
DECLARE
  agency_id_val UUID;
BEGIN
  -- Get agency_id from metadata, default to NestView if not specified
  SELECT id INTO agency_id_val 
  FROM public.agencies 
  WHERE name = COALESCE(NEW.raw_user_meta_data ->> 'agency_name', 'NestView');
